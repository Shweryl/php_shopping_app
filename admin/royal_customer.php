...
$stmp = $pdo->prepare("SELECT * FROM sale_orders WHERE total_price > 1000000 ORDER BY id DESC");
$stmp->execute();
$rawResults = $stmp->fetchAll();
$total_pages = ceil(count($rawResults) / $numOfRecord);

$stmt = $pdo->prepare("SELECT * FROM sale_orders WHERE total_price > 1000000 ORDER BY id DESC LIMIT $offset,$numOfRecord");
$stmt->execute();
